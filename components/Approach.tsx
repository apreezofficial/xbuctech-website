...
export function Approach() {
  return <section className="approach shell" id="approach"><div className="approach-intro"><p className="eyebrow">/ Our approach</p><h2>Because your<br /><em>technology matters.</em></h2><p className="approach-lede">Effective technology management and cybersecurity should work together, not operate as separate functions.</p></div><div className="approach-grid">{approachItems.map((item, index) => <article className="approach-card" key={item.title}><div className="approach-card-top"><span>0{index + 1}</span><div className="approach-icon"><img src={item.image} alt={item.alt} /></div></div><h3>{item.title}</h3><p>{item.text}</p></article>)}</div></section>;
}
